...
void scanTree(TreeNode* &current, TreeNode* root, int value, int &find);
void deleteTree(TreeNode* &current, TreeNode* &treeRoot, int deletedNumber);
void restructure(TreeNode* &root);
void removeNode(TreeNode* &current, TreeNode* &treeRoot);
...
int main(){
  ...
  int loop=1;
  while (loop ==1) {
    cout << endl << "Enter command (add, search, delete, print, listprint, exit):" << endl << "(\"listprint\" is for value/color/parent details, \"print\" is treelike)" << endl;
    cin >> input;
    ...
    if (!strcmp(input, (char*)"delete")){
      ...
      if (find ==1) {
	cout << value << " is in the tree. Deleting " << value << " from the tree. " << endl;
...
void scanTree(TreeNode* &current, TreeNode* root, int value, int &find){
  ...
  if(current->getLeft() != NULL){
    ...
    scanTree(temp, root, value, find);
  }

  //scan right
  if(current->getRight() != NULL){
    TreeNode* temp1 = current->getRight();
    scanTree(temp1, root, value, find);
  }
